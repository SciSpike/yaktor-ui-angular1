(function() {
  ...
  function createElement(name, definition) {
    ...
    if ( definition.hasOwnProperty('enum') ) {
      ...
      element.components={ elements:{}};
      element.components.elements[name]=createElement(name+"JustNess",definition.items);
      ...
      for(prop in props){
        element.components.elements[prop]=createElement(prop+"OhNess",props[prop]);
      }
    }
    return element;
  }
  ...
  function createViews(views) {
    ...
    for (var name in views) {
      ...
      states[stateName].title=titlize(new S(friendly.replace(/.*:state:/,"")).underscore().capitalize().s);
    }
    return states;
  }
