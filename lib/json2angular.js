...
function createScaffold(appname) {
  ...
  [ 'libs','constant', 'service', 'partial', 'controller','controller/custom', 'gruntTasks','locale',"modules","styles", "fonts", "less"].forEach(
      function(directory) {
...
function createProject(spec) {
  ...
  fs.readdirSync(path.join(__dirname, '../', 'template', 'styles')).forEach(function(file) {
    createTemplate(appname, path.join('styles', file),null,dummyRenderer);
  });
  ...
  var doStuff = function() {
    ...
    createTemplate(appname, 'package.json');
    ...
    fs.readdirSync(path.join(__dirname, '../', 'template', 'less')).forEach(function(folder) {
      fs.mkdir(path.join(path.resolve(appname), 'less', folder), function(response){
        
        fs.readdirSync(path.join(__dirname, '../', 'template', 'less', folder)).forEach(function(file) {
          /* BIG ASSUMPTION HERE */
          if(file == 'custom'){
            !fs.existsSync( path.join(appname,'less', folder, file)) &&
            fs.mkdirSync(path.join(path.resolve(appname), 'less', folder, file));
            fs.readdirSync(path.join(__dirname, '../', 'template', 'less', folder, file)).forEach(function(customFile) {
              !fs.existsSync( path.join(appname,'less', folder, file, customFile)) &&
              createTemplate(appname, path.join('less', folder, file, customFile), {}, ejs.render);
            });
          }else{
            createTemplate(appname, path.join('less', folder, file), {}, ejs.render);
          }
        });
       });   
    });
