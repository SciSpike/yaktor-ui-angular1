...
underscore.mixin(require('underscore.deep'));
/*
...
function createScaffold (appname) {
  ...
  [ 'modules', 'modules/crud', 'modules/agents', 'shared', 'shared/controllers', 'shared/directives', 'shared/services', 'shared/locale', 'gruntTasks', 'libs', 'libs/vendor', 'styles', 'styles/css', 'partials', 'partials/crud', 'partials/agents', 'clientConfig' ]
...
function createAgentChildState (appname, moduleName, state, parentStateName, moduleUrl) {
  var controllerPath = path.join(appname, 'modules', 'agents', moduleName, 'controllers'), partialPath = path.join(appname,
      'partials', 'agents', moduleName);
  ...
  fs.mkdir(path.join(partialPath, parentStateName), function (response) {
    createTemplate(appname, path.join('modules', 'actionableAgent.html'), {
      moduleName : moduleName,
      parentStateName : parentStateName,
      state : state,
      moduleUrl: moduleUrl,
    ...
    var oldHtmlPath = path.join(appname, 'modules', 'actionableAgent.html'), newHtmlPath = path.join(partialPath,
...
function createAgentState (appname, moduleName, state, parentUrl) {
  ...
  var oldControllerPath = path.join(appname, 'modules', 'agentController.js'), newControllerPath = path.join(appname,
      'modules', 'agents', moduleName, 'controllers', state.name + '.js');
  fs.renameSync(oldControllerPath, newControllerPath);
  if (state.elements) {
  ...
  var oldHtmlPath = path.join(appname, 'modules', 'state.html'), newHtmlPath = path.join(appname, 'partials', 'agents', 
...
function createAgent (appname, masterSpec, moduleSpec, cb) {
  fs.mkdir(path.join(path.resolve(appname), 'modules', 'agents', moduleSpec.name), function (response) {
    ...
    var oldPath = path.join(appname, 'modules', 'agentModule.js'), newPath = path.join(appname, 'modules', 'agents',
    ...
    fs.mkdir(path.join(path.resolve(appname), 'modules', 'agents', moduleSpec.name, 'controllers'), function (response) {
      ...
      var oldControllerPath = path.join(appname, 'modules', 'agentMasterController.js'), newControllerPath = path.join(
          appname, 'modules', 'agents', moduleSpec.name, 'controllers', moduleSpec.name + '.js');
      fs.renameSync(oldControllerPath, newControllerPath);
      ...
      fs.mkdir(path.join(path.resolve(appname), 'partials', 'agents', moduleSpec.name), function (response) {
        ...
        var oldHtmlPath = path.join(appname, 'modules', 'index.html'), newHtmlPath = path.join(appname, 'partials', 'agents', 
        ...
        fs.mkdir(path.join(path.resolve(appname), 'modules', 'agents', moduleSpec.name, 'services'), function (response) {
          ...
          var oldServicePath = path.join(appname, 'modules', 'agentServices.js'), newServicePath = path.join(appname,
              'modules', 'agents', moduleSpec.name, 'services', moduleSpec.name + 'Services.js');
