(function() {
  ...
  // these elements.
  function createActionableElement(ui) {
  ...
  function createActionable(name, description) {
    ...
    var fragments = [];
    var data = {name: name.humanize().s};
    var ui = {tag: "h3", html: "${name}"};
    fragments.push(json2html.transform({name: name.humanize().s}, {tag: "h3", html: "${name}"}));
    ...
    for (var el in elements) {
      ...
      data = {state: name.s, model: el, name: name.humanize().s};
      ui = createActionableElement( fragmentDefinition.ui );
