...
function createCrudState (appname, masterSpec, moduleName, state, parentStateName, moduleUrl, agents) {
	  ...
	  var contents = getTemplateContents(path.join('modules', 'crud', crudType + 'Controller.js'), {
	  ...
	  createTemplate(appname, path.join('modules', 'crud', 'crudController.js'), {
		  ...
		  agents : agents,
		  agentSpec: masterSpec.agents,
		  crudControllerContents: contents
	  });
...
function createAgentChildState (appname, moduleName, state, parentStateName, moduleUrl) {
	  ...
	  fs.mkdir(path.join(controllerPath, parentStateName), function (response) {
	    createTemplate(appname, path.join('modules', 'agent', 'agentChildController.js'), {
	      moduleName : moduleName,
	      state : state
	    });
	    var oldControllerPath = path.join(appname, 'modules', 'agent', 'agentChildController.js'),
	  ...
	  fs.mkdir(path.join(partialPath, parentStateName), function (response) {
	    createTemplate(appname, path.join('modules', 'agent', 'agentChild.html'), {
	      moduleName : moduleName,
	      parentStateName : parentStateName,
	      state : state,
	      moduleUrl: moduleUrl,
	    ...
	    var oldHtmlPath = path.join(appname, 'modules', 'agent', 'agentChild.html'), newHtmlPath = path.join(partialPath,
...
function createAgentState (appname, moduleName, state, parentUrl) {
	  ...
	  createTemplate(appname, path.join('modules', 'agent', 'agentController.js'), {
	    moduleName : moduleName,
	    state : state,
	    parentUrl : parentUrl
	  });
	  
	  var oldControllerPath = path.join(appname, 'modules', 'agent', 'agentController.js'),
	  ...
	  createTemplate(appname, path.join('modules', 'agent', 'agent.html'), {
	    moduleName : moduleName,
	    state : state,
	    parentUrl: parentUrl
	  });
	  
	  var oldHtmlPath = path.join(appname, 'modules', 'agent', 'agent.html'),
...
function createAgent (appname, masterSpec, moduleSpec, cb) {
  ...
  fs.mkdir(path.join(path.resolve(appname), 'modules', 'agents', moduleSpec.name), function (response) {
	    createTemplate(appname, path.join('modules', 'agent', 'agentModule.js'), {
	      moduleName : moduleSpec.name,
	      states : moduleSpec.states
	    });
	    
	    var oldPath = path.join(appname, 'modules', 'agent', 'agentModule.js'),
	    ...
	    fs.mkdir(path.join(path.resolve(appname), 'modules', 'agents', moduleSpec.name, 'controllers'), function (response) {
	      createTemplate(appname, path.join('modules', 'agent', 'agentMasterController.js'), {
	        moduleName : moduleSpec.name,
	        actions : moduleSpec.actions,
	        states : moduleSpec.states
	      });
	      var oldControllerPath = path.join(appname, 'modules', 'agent', 'agentMasterController.js'),
	      ...
	      fs.mkdir(path.join(path.resolve(appname), 'partials', 'agents', moduleSpec.name), function (response) {
	        ...
	        fs.mkdir(path.join(path.resolve(appname), 'modules', 'agents', moduleSpec.name, 'services'), function (response) {
	          createTemplate(appname, path.join('modules', 'agent', 'agentServices.js'), {
	            moduleName : moduleSpec.name,
	            actions : moduleSpec.actions,
	            states : moduleSpec.states,
	            agentSpec: masterSpec.agents 
	          });
	          var oldServicePath = path.join(appname, 'modules', 'agent', 'agentServices.js'),
