...
---------------- */
	...
	function createProject(spec) {
		...
		copyFiles = function(theme) { //CALLED FROM setupDependencies FUNCTION
			var moduleNames = {
					crud: [],
					agents: []
			};
	        ...
	        fs.readdirSync(path.join(__dirname, '../', 'template', 'gruntTasks')).forEach(function(file) {
	        	checkIfFolder(appname, 'gruntTasks', file, {moduleNames: moduleNames, locale: locale, theme: theme});
	        });
	        ...
	        fs.readdirSync(path.join(__dirname, '../', 'template', 'clientConfig')).forEach(function(file) {
	        	fs.exists(path.join(appname, 'clientConfig', file), function(exists) {
	        		if (!exists) {
	        		    ...
	        		    checkIfFolder(appname, 'clientConfig', file, moduleData);
	        		}
	        	});
			});
		}
			            	...
			            	function readConfig() { 
			            	    fs.readFile(path.join(appname, 'clientConfig', 'config.json'), 'utf-8', function (err, content) {
			            	        if(err){
			            	        	...
			            	        	var theme = JSON.parse(content).theme;
			            	        }
			            	        copyFiles(theme); //EXISTS INSIDE createProject FUNCTION
			            	    });
			            	}
			            	readConfig();
			            }
