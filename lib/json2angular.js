(function() {
  ...
  function createActionable(stateName, description, rest) {
    ...
    data = { stateName: stateName.s, html: json2html.transform(data, ui) };
    ui = description.ui;
  ...
  function createState(appname, statename, description, layout, rest) {
    ...
    for (var name in elements) {
      ...
      var actionableHTML = createActionable(name.s, elements[name.s], rest);
      createTemplate(appname, path.join('partial', 'actionable.html'), {html: actionableHTML});
