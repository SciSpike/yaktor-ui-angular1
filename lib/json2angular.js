...
---------------- */
	...
	function createActionableController(appname, moduleName, state, folderPath){
		...
		var oldControllerPath = path.join(appname, 'modules', 'actionableController.js'),
			newControllerPath = path.join(folderPath, state.name + '.js');
		fs.renameSync(oldControllerPath, newControllerPath);
	}
	
	function createController(appname, moduleName, state, folderPath){
	...
	function createChildState(appname, moduleName, state, parentStateName){
		...
		fs.mkdir(path.join(controllerPath, parentStateName), function(response){
			...
			createActionableController(appname, moduleName, state, moduleFolderPath);
		});
		for(obj in state.components.elements){
			console.log('----- 1 ------');
			console.log(state.components.elements[obj]);
			console.log('----- 2 ------');
			console.log(state.components.elements[obj].ui);
		}
	...
	function createState(appname, moduleName, state) {
		var controllerPath =  path.join(appname, 'modules', moduleName, 'controllers'),
			partialPath =  path.join(appname, 'partials', moduleName);
		createController(appname, moduleName, state, controllerPath);
...
//	{
//		"proto":"ws://",
//		"url":"/SodaPurchase.purchaser.hasMoney",
//		"elements":{
//			"spendMoney":{
//				"ui":{
//					"tag":"div",
