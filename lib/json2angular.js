(function() {
  ...
  function createActionable(stateName, description) {
    ...
    // Append fragments to parent div
    data = { html: fragments.join('')};
    ui = {tag: "div", 'class': "well", html: "${html}"};
    
    
    // Append fragments to parent node
    data = { html: json2html.transform(data, ui) };
    ui = description.ui;
    ui.class = "actionable col-md-6";
    ui.html = "${html}";
    
    return json2html.transform(data, ui);
  }
  
  function createLayout(view, statename, description, layout) {
    var states = Object.keys(description.elements);
    states = states.map(function(d) { return '"' + d + '"'; }).join(", ");
    ...
    view["tabs"] = json2html.transform(data, ui);
    return view;
  ...
  function createState(appname, statename, description, layout) {
    ...
    view = createLayout(view, statename, description, layout);
  ...
  function createApp(appname, states, layout) {
    
    for (var name in states) {
      var description = states[name];
      createState(appname, name, description, layout);
    }
  ...
  function createProject(spec) {
    ...
    createApp(appname, states, layout);
  };
