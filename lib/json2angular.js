			...
			fs.mkdir(path.join(path.resolve(appname), 'modules', moduleSpec.name, 'controllers'), function(response){
				...
				fs.mkdir(path.join(path.resolve(appname), 'partials', moduleSpec.name), function(response){
					...
					fs.mkdir(path.join(path.resolve(appname), 'modules', moduleSpec.name, 'services'), function(response){
						...
						var oldServicePath = path.join(appname, 'modules', 'agentServices.js'),
								newServicePath = path.join(appname, 'modules', moduleSpec.name, 'services', moduleSpec.name + 'Services.js');
		...
		fs.mkdir(path.join(path.resolve(appname), 'modules', 'crud', crudSpec.name, 'controllers'), function(response){
			...
			fs.mkdir(path.join(path.resolve(appname), 'partials', 'crud', crudSpec.name), function(response){
				...
				fs.mkdir(path.join(path.resolve(appname), 'modules', 'crud', crudSpec.name, 'services'), function(response){
					...
					var oldServicePath = path.join(appname, 'modules', 'crudServices.js'),
...
---------------- */
	...
	function createApp(appname, spec, cb) {
		var moduleNames = [];
...
---------------- */
	...
	function createProject(spec, cb) {
		...
		var locale = process.env.LANG?process.env.LANG.replace(/\..*/,""):"en_US";
		var moduleData = {moduleNames: moduleNames, locale: locale};
		
	    createTemplate(appname, path.join('Gruntfile.js'), moduleData);
	    async.series([
		    function(cb){
		    	createApp(appname, spec.modules, cb);
		    },
		    function(cb){
		    ...
		    function(cb){
		    	if(isNew){
			    	...
			    	async.each(files, function(file, cb){
			    		checkIfFolder(appname, 'clientConfig', file, moduleData, cb);
			    	}, cb);
		    	}else{
		    		cb();
		    	}
		    ...
		    function(cb){
			    fs.readFile(path.join(appname, 'clientConfig', 'config.json'), 'utf-8', function (err, content) {
	            	if(err){
	            	    console.log(err);
	            	}
		    ...
		    function(cb){
		    	...
		    	async.each(files, function(file, cb){
		    		checkIfFolder(appname, 'gruntTasks', file, {moduleNames: moduleNames, locale: locale, theme: theme}, cb);
		    	}, cb);
		    },
		    function(cb){
		    	var files = fs.readdirSync(path.join(__dirname, '../', 'template', 'styles'));
		    	async.each(files, function(file, cb){
		    		checkIfFolder(appname, 'styles', file, null, cb);
		    	}, cb);
		    },
		    function(cb){
		    ...
		    function(cb){
		    	exec('npm',['run', 'grunt'], path.resolve(appname),
					function (error) {
						if (error) {
...
---------------- */
	function setupDependencies(appname, cb){
		async.series([
			function(cb){
				exec('npm',['install'], path.resolve(appname), function (error) {
					if (error) {
					   console.log('exec error: ' + error);
					}
					cb();      
