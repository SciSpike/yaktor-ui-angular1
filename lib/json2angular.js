...
// configurations for
// these elements.
function createActionableElement(stateName, elementName,definition,  rest) {
  var fragmentDir, fragmentPath, fileExists, source, view, options
      type= definition.type, 
      ui = definition.ui, 
      subComponents = definition.components;
...
function createActionable(stateName, description, rest) {
  ...
  for ( var name in elements) {
    ...
    fragments.push({
      label : name,
      fragment : createActionableElement(stateName, name, definition, rest)
    });
  }
