(function() {
  ...
  function createScaffold(appname, states) {
    ...
    ['constant', 'service', 'partial'].forEach(function(directory) {
  ...
  function createActionableElement(stateName, elementName, type, ui) {
    ...
    // Or create a fragment based on the type
    // TODO: Combine both ui and type definitions
    if (type) {
      switch (type) {
      ...
      case "checkbox":
        ...
        ui["ng-repeat"] = "option in [" + options + "]";
        delete ui.options;
      ...
      case "enum":
        ...
        var options = ui.options || [];
        options = options.map(function(d) { return '"' + d + '"'; }).join(", ");
        ui["ng-options"] = "option for option in [" + options + "] track by $index";
        delete ui.options;
  ...
  function createActionable(stateName, description) {
    ...
    fragments.push(json2html.transform({stateName: stateName.humanize().s}, {tag: "h3", html: "${stateName}"}));
    ...
    for (var name in elements) {
      ...
      fragmentDefinition = elements[name];
      
      // data = {state: stateName.s, model: name, stateName: stateName.humanize().s};
      // ui = createActionableElement( fragmentDefinition.type, fragmentDefinition.ui );
      // var fragment = json2html.transform(data, ui);
      
      var fragment = createActionableElement(stateName, name, fragmentDefinition.type, fragmentDefinition.ui);
