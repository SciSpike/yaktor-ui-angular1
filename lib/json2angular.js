		...
		copyFiles = function() { //CALLED FROM setupDependencies FUNCTION
			...
			for(var module in spec.modules){
				for(var i=spec.modules[module].length-1; i>=0; i--){
					//console.log(spec.modules[module][i].name.split('/').slice(-1)[0]);
					//moduleNames.push(spec.modules[module][i].name);
					moduleNames.push(spec.modules[module][i].name.split('/').slice(-1)[0]);
				}
			}
