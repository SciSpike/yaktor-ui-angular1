 ...
 * EVERY YAKTOR-UI PROJECT ------------------- ----------------
 */
function createScaffold (appname) {
  !fs.existsSync(appname) && fs.mkdirSync(appname);
  var projectPath = path.join(__dirname, appname);
  [ 'modules', 'modules/crud', 'modules/agents', 'shared', 'shared/controllers', 'shared/directives', 'shared/services', 'shared/locale', 'gruntTasks', 'libs', 'libs/vendor', 'styles', 'styles/css', 'partials', 'partials/crud', 'partials/agents', 'clientConfig', 'cordova_template' ]
...
var specModel = {
	  ...
	  var crudAgent = getUnrenderedTemplateContents(path.join('modules', 'crud', 'crudAgents.js'));
	  var contents = getUnrenderedTemplateContents(path.join('modules', 'crud', crudType + 'Controller.js'));    
    var controllerContents = mergeTemplateAndModel(crudAgent+ contents, specModel);
    
	  createTemplate(appname, path.join('modules', 'crud', 'crudController.js'), {
		  parentStateName : parentStateName,
		  moduleName : moduleName,
		  state : state,
		  agents : agents,
	  ...
	  var oldControllerPath = path.join(appname, 'modules', 'crud', 'crudController.js'),
	  	  newControllerPath = path.join(controllerPath, moduleName + '.js');  
	  fs.renameSync(oldControllerPath, newControllerPath);

	  createTemplate(appname, path.join('modules', 'crud', crudType + '.html'), {
	    moduleName : moduleName,
	    parentStateName : parentStateName,
	    state : state,
	    moduleUrl: moduleUrl,
	  ...
	  var oldHtmlPath = path.join(appname, 'modules', 'crud', crudType + '.html'),
...
function createAgentChildState (appname, moduleName, state, parentStateName, moduleUrl) {
	  var controllerPath = path.join(appname, 'modules', 'agents', moduleName, 'controllers'),
	  	  partialPath = path.join(appname,'partials', 'agents', moduleName);
	  
	  state['name'] = state.subPath.replace('/', '');
	  
	  fs.mkdir(path.join(controllerPath, parentStateName), function (response) {
	    createTemplate(appname, path.join('modules', 'agents', 'agentChildController.js'), {
	      moduleName : moduleName,
	      state : state
	    });
	    var oldControllerPath = path.join(appname, 'modules', 'agents', 'agentChildController.js'),
	  ...
	  fs.mkdir(path.join(partialPath, parentStateName), function (response) {
	    createTemplate(appname, path.join('modules', 'agents', 'agentChild.html'), {
	      moduleName : moduleName,
	      parentStateName : parentStateName,
	      state : state,
	      moduleUrl: moduleUrl,
	    ...
	    var oldHtmlPath = path.join(appname, 'modules', 'agents', 'agentChild.html'), newHtmlPath = path.join(partialPath,
...
function createAgentState (appname, moduleName, state, parentUrl) {
	  ...
	  createTemplate(appname, path.join('modules', 'agents', 'agentController.js'), {
	    moduleName : moduleName,
	    state : state,
	    parentUrl : parentUrl
	  });
	  
	  var oldControllerPath = path.join(appname, 'modules', 'agents', 'agentController.js'),
	  	  newControllerPath = path.join(appname,'modules', 'agents', moduleName, 'controllers', state.name + '.js');
	  ...
	  createTemplate(appname, path.join('modules', 'agents', 'agent.html'), {
	    moduleName : moduleName,
	    state : state,
	    parentUrl: parentUrl
	  });
	  
	  var oldHtmlPath = path.join(appname, 'modules', 'agents', 'agent.html'),
	  	  newHtmlPath = path.join(appname, 'partials', 'agents', moduleName, state.name + '.html');
	  fs.renameSync(oldHtmlPath, newHtmlPath);
}
...
function createAgent (appname, masterSpec, moduleSpec, cb) {
  ...
  fs.mkdir(path.join(path.resolve(appname), 'modules', 'agents', moduleSpec.name), function (response) {
	    createTemplate(appname, path.join('modules', 'agents', 'agentModule.js'), {
	      moduleName : moduleSpec.name,
	      states : moduleSpec.states
	    });
	    
	    var oldPath = path.join(appname, 'modules', 'agents', 'agentModule.js'),
	    	newPath = path.join(appname, 'modules', 'agents', moduleSpec.name, moduleSpec.name + '.js');
	    fs.renameSync(oldPath, newPath);
	
	    // CREATE THE AGENT MAIN CONTROLLER
	    fs.mkdir(path.join(path.resolve(appname), 'modules', 'agents', moduleSpec.name, 'controllers'), function (response) {
	      createTemplate(appname, path.join('modules', 'agents', 'agentMasterController.js'), {
	        moduleName : moduleSpec.name,
	        actions : moduleSpec.actions,
	        states : moduleSpec.states
	      });
	      var oldControllerPath = path.join(appname, 'modules', 'agents', 'agentMasterController.js'),
	      	  newControllerPath = path.join(appname, 'modules', 'agents', moduleSpec.name, 'controllers', moduleSpec.name + '.js');
	      fs.renameSync(oldControllerPath, newControllerPath);
	
	      // CREATE THE MODULE INIT STATE CONTROLLER AND PARTIAL
	      fs.mkdir(path.join(path.resolve(appname), 'partials', 'agents', moduleSpec.name), function (response) {
	        createTemplate(appname, path.join('modules', 'index.html'), {});
	        
	        var oldHtmlPath = path.join(appname, 'modules', 'index.html'),
	        	newHtmlPath = path.join(appname, 'partials', 'agents', moduleSpec.name, 'index.html');
	        fs.renameSync(oldHtmlPath, newHtmlPath);
	        ...
	        fs.mkdir(path.join(path.resolve(appname), 'modules', 'agents', moduleSpec.name, 'services'), function (response) {
	          createTemplate(appname, path.join('modules', 'agents', 'agentServices.js'), {
	            moduleName : moduleSpec.name,
	            actions : moduleSpec.actions,
	            states : moduleSpec.states,
	            agentSpec: masterSpec.agents 
	          });
	          var oldServicePath = path.join(appname, 'modules', 'agents', 'agentServices.js'),
	          	  newServicePath = path.join(appname, 'modules', 'agents', moduleSpec.name, 'services', moduleSpec.name + 'Services.js');
...
function createCrud (appname, masterSpec, crudSpec, cb) {
	  fs.mkdir(path.join(path.resolve(appname), 'modules', 'crud', crudSpec.name), function (response) {
	    createTemplate(appname, path.join('modules', 'crud', 'crudModule.js'), {
	    ...
	    var oldPath = path.join(appname, 'modules', 'crud', 'crudModule.js'),
	    ...
	    fs.mkdir(path.join(path.resolve(appname), 'modules', 'crud', crudSpec.name, 'controllers'), function (response) {
	      createTemplate(appname, path.join('modules', 'crud', 'crudMasterController.js'), {
	        moduleName : crudSpec.name,
	        actions : crudSpec.actions
	      });
	      
	      var oldControllerPath = path.join(appname, 'modules', 'crud', 'crudMasterController.js'),
	      ...
	      fs.mkdir(path.join(path.resolve(appname), 'partials', 'crud', crudSpec.name), function (response) {
	        ...
	        fs.mkdir(path.join(path.resolve(appname), 'modules', 'crud', crudSpec.name, 'services'), function (response) {
	          createTemplate(appname, path.join('modules', 'crud', 'crudServices.js'), {
	            moduleName : crudSpec.name,
	            actions : crudSpec.actions,
	            agents: crudSpec.agents,
	            agentSpec: masterSpec.agents 
	          ...
	          var oldServicePath = path.join(appname, 'modules', 'crud', 'crudServices.js'),
