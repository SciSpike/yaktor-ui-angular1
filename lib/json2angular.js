...
function renderTemplate (filename,data,renderer){
  ...
  data.compileDebug=compileDebug;
...
function createScaffold(appname) {
  ...
  [ 'constant', 'service', 'partial', 'controller','controller/custom', 'gruntTasks' ].forEach(
      function(directory) {
  ...
  for ( var n in elements) {
    ...
    if(!fs.existsSync(newPath)){
      createTemplate(appname, path.join('controller', 'emptyController.js'), {
        actionableName:n,
        contents : element.components?createContents(element.components.elements,n):"",
            actionable:element,
            actionables:description
...
function createProject(spec) {
  ...
  fs.readdirSync(path.join(__dirname, '../', 'template', 'gruntTasks')).forEach(function(file) {
	  createTemplate(appname, path.join('gruntTasks', file));
  });
